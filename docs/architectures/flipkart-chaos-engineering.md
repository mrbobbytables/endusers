...
sidebar_label: "Flipkart Internet Pvt. Ltd."
---

import CNCFProjectCard from '@site/src/components/CNCFProjectCard';

> Imported from the [CNCF Cloud Native Reference Architecture project](https://github.com/cncf/architecture/tree/3ddf917facd0aea9694cc9c12c52e583fd2f86cb/content/en/architectures/flipkart-chaos-engineering). Source revision: `3ddf917facd0aea9694cc9c12c52e583fd2f86cb`. Documentation is distributed under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/).

## Relevant CNCF projects

  <CNCFProjectCard name={"LitmusChaos"} href={"https://www.cncf.io/projects/litmus/"} logo={"/img/cncf-projects/litmus-litmus-icon-color.svg"} since={"2023"} description={"Central chaos engineering platform. Provides Kubernetes-native chaos orchestration, a workflow controller, a probe framework, a drag-and-drop UI, and resilience scoring. Flipkart deploys a single centralized control plane (operator \\+ workflow controller) and federates tenant access through subscribers — forming the foundation of the multi-tenant chaos platform."} />

  <CNCFProjectCard name={"Kubernetes"} href={"https://www.cncf.io/projects/kubernetes/"} logo={"/img/cncf-projects/kubernetes-kubernetes-icon-color.svg"} since={"2021"} description={"Runtime substrate for hundreds of Flipkart microservices and for the LitmusChaos control plane itself. Custom Resource Definitions (ChaosEngine, ChaosExperiment, ChaosResult) model chaos as Kubernetes objects. DaemonSets are used to provide a node-local, long-lived chaos injection surface for high-availability experiment execution."} />

  <CNCFProjectCard name={"Argo Workflows"} href={"https://www.cncf.io/projects/argo/"} logo={"/img/cncf-projects/argo-argo-icon-color.svg"} since={"2023"} description={"Underlying workflow engine used by Litmus to model multi-step chaos experiments — sequencing pre-checks, fault injection, probes, and recovery validation as a DAG. Flipkart’s Script Runner fault plugs into this DAG to enable dynamic target selection and context chaining between steps."} />

  <CNCFProjectCard name={"Prometheus"} href={"https://www.cncf.io/projects/prometheus/"} logo={"/img/cncf-projects/prometheus-prometheus-icon-color.svg"} since={"2021"} description={"Metrics backbone for both production workloads and chaos experiments. Chaos drills are validated against Prometheus-driven SLO and alert thresholds (CPU saturation, 5xx error rates, latency, queue depth). Litmus’s Prometheus probes assert hypotheses during fault injection."} />

  <CNCFProjectCard name={"Helm"} href={"https://www.cncf.io/projects/helm/"} logo={"/img/cncf-projects/helm-helm-icon-color.svg"} since={"2022"} description={"Used to package and roll out the centralized Litmus control plane, tenant subscribers, the DaemonSet-based injection layer, and Flipkart’s custom fault images consistently across staging and production clusters."} />

    <CNCFProjectCard name={"containerd"} href={"https://www.cncf.io/projects/containerd/"} logo={"/img/cncf-projects/containerd-containerd-icon-color.svg"} since={"2021"} description={"Container runtime on Flipkart’s Kubernetes nodes. The DaemonSet injection model relies on stable runtime primitives to execute parallel chaos sessions safely on a single node."} />
...
![](/img/architectures/flipkart-chaos-engineering/top-level-diagram.png)

The platform is organized into four logical layers:

![](/img/architectures/flipkart-chaos-engineering/layers.png)
